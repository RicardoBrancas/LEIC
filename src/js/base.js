...
function create_material(name, color, {emissive = false, shininess = 30, specular = 0x111111, transparent = false, opacity = 1} = {}) {
	...
	let phong = new THREE.MeshPhongMaterial({color: color, emissive: emissive ? color : 0x0, shininess: shininess, specular: specular, transparent: transparent, opacity: opacity});
	phong.name = name;
	phong_materials.set(name, phong);

	let lambert = new THREE.MeshLambertMaterial({color: color, emissive: emissive ? color : 0x0, transparent: transparent, opacity: opacity});
	lambert.name = name;
